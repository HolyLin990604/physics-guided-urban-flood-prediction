...
flowchart TD
    ...
    C["Strong Physics Boundary<br/>Level 4+ proxy diagnostics supported<br/>Level 5 SWE/PINN not supported<br/>Phases 26 + 30-47"]:::boundary
    ...
    E["Current Decision<br/>controlled 128x128 baseline complete<br/>Level 4+ route viable<br/>Phase 47 complete"]:::current
    F["Next Research Direction<br/>Phase 48 diagnostics or baseline review<br/>no uncontrolled expansion<br/>no Level 5 claim"]:::current
...
The next justified follow-up is conservative Phase 48 full-dataset reliability and physical proxy diagnostics, or a formal Phase 47 baseline review before any expansion. Phase 47 completed the controlled UrbanFlood24 full-dataset `128 x 128` downsample `seed42` 10e baseline pilot with `selected_decision = phase47_controlled_128_downsample_seed42_pilot_completed`, `best_test_rmse = 0.01109213042097205`, `test_mae = 0.00525291082279485`, `test_wet_dry_iou = 0.8255524213115374`, `test_rollout_stability = 0.998722607580324`, and `test_step_rmse_std = 0.0012824604989987165`. This establishes a viable Level 4+ full-dataset `128 x 128` baseline route only. It does not support Level 5, SWE/PINN, strict conservation, full mass conservation, or hydrodynamic closure claims, and it does not authorize uncontrolled expansion to `seed123` / `seed202`, `256 x 256`, tile, multiscale, full `500 x 500`, sweeps, or new loss redesign. The current Phase 10 boundary-band setting remains `boundary_band_pixels = 1` and `boundary_weight = 2.0`, with Phase 25 target-wet recall consistency as a strong three-seed positive candidate and Phase 47 as the current controlled Level 4+ full-dataset baseline boundary. Phase 27 remains a mixed seed42 pilot whose primary volume-response objective was not confirmed, Phase 28 remains the diagnostic explanation for why Phase 27 should not be directly expanded, Phase 29 remains a mixed tolerance-band seed42 pilot whose trade-off is not acceptable for confirmation, Phase 39 shows that the `manhole_nonzero_false_dry_guardrail` target/proxy behavior is insufficient under broader guardrails, and Phase 44-47 keep short-term Level 5/SWE/PINN claims frozen.
...
- do not expand Phase 47 to `seed123` / `seed202`, `256 x 256`, tile, multiscale, full `500 x 500`, sweeps, or new loss redesign without a formal review and plan
- follow the compact Phase 45-50 route: Phase 45 full dataset indexing and lightweight adapter complete; Phase 46 dataloader smoke test and downsample/tiling feasibility complete; Phase 47 controlled full dataset downsample baseline complete; Phase 48 full dataset reliability and physical proxy diagnostics or formal Phase 47 baseline review; Phase 49 full dataset warning framework extension; Phase 50 optional expansion only after reviewed evidence
