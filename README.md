...
flowchart TB
    ...
    H["Current Status<br/>Phase 46 dataloader smoke complete<br/>UrbanFlood24 full route<br/>Phase 47 baseline plan next<br/>Level 5 SWE/PINN frozen"]:::current
...
flowchart TD
    ...
    C["Strong Physics Boundary<br/>Level 4+ proxy diagnostics supported<br/>Level 5 SWE/PINN not supported<br/>Phases 26 + 30-46"]:::boundary
    ...
    E["Current Decision<br/>UrbanFlood24 dataloader smoke ready<br/>no training or Level 5 claim<br/>Phase 46 complete"]:::current
    F["Next Research Direction<br/>reviewed Phase 47 plan<br/>controlled 128x128 baseline first<br/>then gated Level 4+ proxy diagnostics"]:::current
...
- **Phase 47 may now be planned as a controlled full dataset downsample baseline, likely starting with `128 x 128`, but training remains unauthorized until that reviewed plan exists**
- **Phase 45-50 roadmap: Phase 45 full dataset indexing and lightweight adapter complete; Phase 46 dataloader smoke test and downsample/tiling feasibility complete; Phase 47 controlled full dataset downsample baseline plan; Phase 48 full dataset reliability and physical proxy diagnostics; Phase 49 full dataset warning framework extension; Phase 50 optional 256x256 / tile / multiscale refinement**
...
The next justified follow-up is a reviewed Phase 47 controlled full dataset downsample baseline training plan, likely starting with `128 x 128`, after Phase 46 established a viable no-training dataloader smoke path. Phase 47 planning should not be unreviewed training, metadata recovery, author-side clarification, ICM/MIKE+ export, seed expansion, a sweep, further proxy-loss redesign, SWE loss implementation, SWE residual implementation, PINN implementation, or post-hoc rescue of the rejected pilot. Phase 38 completed the single authorized `seed42` training run, test evaluation, and guardrail evaluation, with `final_decision = seed42_pilot_rejected`; Phase 39 then completed diagnostic-only trade-off analysis with `phase39_decision = tradeoff_diagnosis_completed_with_missing_optional_inputs`; Phase 40 selected `pause_loss_redesign_move_to_swe_data_readiness`; Phase 41 completed the SWE data readiness audit with `readiness_decision = readiness_uncertain_requires_external_data_export`, `level5_supported = false`, and `level4_proxy_supported = true`; Phase 42 completed the hydrodynamic export requirement specification without implementing SWE/PINN or claiming Level 5 support; Phase 43 inspected UrbanFlood24 full with `level5_supported = false`, `level4_plus_supported = true`, and `training_authorized = false`; Phase 44 replanned the route around UrbanFlood24 full Level 4+ work only; Phase 45 created a reproducible full-dataset index with `selected_decision = indexing_ready_for_dataloader_smoke` and `training_authorized = false`; and Phase 46 completed no-training dataloader smoke/downsample/tiling checks with `selected_decision = dataloader_smoke_ready_for_downsample_baseline` and `training_authorized = false`. The current Phase 10 boundary-band setting remains `boundary_band_pixels = 1` and `boundary_weight = 2.0`, with Phase 25 target-wet recall consistency as a strong three-seed positive candidate and Phase 46 as the current Level 4+ full-dataset dataloader feasibility boundary. Phase 27 remains a mixed seed42 pilot whose primary volume-response objective was not confirmed, Phase 28 remains the diagnostic explanation for why Phase 27 should not be directly expanded, Phase 29 remains a mixed tolerance-band seed42 pilot whose trade-off is not acceptable for confirmation, Phase 39 shows that the `manhole_nonzero_false_dry_guardrail` target/proxy behavior is insufficient under broader guardrails, and Phase 44-46 keep short-term Level 5/SWE/PINN claims frozen.
...
- plan Phase 47 as a controlled full dataset downsample baseline, likely starting with `128 x 128`
- keep training unauthorized until a reviewed Phase 47 baseline plan exists
