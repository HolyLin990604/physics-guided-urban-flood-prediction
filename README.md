...
flowchart TB
    ...
    H["Current Status<br/>Phase 51 decision complete<br/>Phase 52 controlled 128x128 seed42 longer run authorized<br/>no Phase 51 training<br/>Level 5 SWE/PINN frozen"]:::current
...
Phase 51 completed a reviewed expansion decision after the Phase 50 synthesis and figure support. See [Phase 51 findings](docs/phase51_reviewed_expansion_decision_findings.md), [selected decision](analysis/phase51_reviewed_expansion_decision/phase51_selected_decision.md), [selected decision JSON](analysis/phase51_reviewed_expansion_decision/phase51_selected_decision.json), [expansion option matrix](analysis/phase51_reviewed_expansion_decision/phase51_expansion_option_matrix.csv), and [risk assessment](analysis/phase51_reviewed_expansion_decision/phase51_risk_assessment.csv). The decision is `selected_decision = phase51_authorize_128x128_seed42_longer_run`; `authorized_next_phase = phase52_controlled_128x128_seed42_longer_run_baseline`; `no_training_in_phase51 = true`; `level4_plus_route_supported = true`; `level5_supported = false`; `no_swe_pinn = true`; and `no_uncontrolled_expansion = true`. Phase 51 itself did not run training. Phase 52 is the only authorized next training phase: a separate controlled `128 x 128` `seed42` longer-run baseline with a recommended 40-epoch cap and direct comparison against the Phase 47 `128 x 128` `seed42` 10e baseline. Seed replication, `256 x 256`, tile, multiscale, full `500 x 500`, sweeps, new loss redesign, model/loss/config architecture changes, SWE/PINN, Level 5, strict conservation, full mass conservation, hydrodynamic closure, calibrated probabilities, production readiness, and uncontrolled training expansion remain outside this authorization.
...
flowchart TD
    ...
    E["Current Decision<br/>Phase 51 review complete<br/>Phase 52 controlled longer run authorized<br/>no Phase 51 training"]:::current
...
- **Phase 52 is the only authorized next training phase and is limited to a controlled `128 x 128` `seed42` longer-run baseline with a recommended 40-epoch cap and direct Phase 47 comparison**
...
The next justified follow-up is the separately controlled Phase 52 `128 x 128` `seed42` longer-run baseline authorized by Phase 51. Phase 51 was decision-only and ran no training. Its decision is `selected_decision = phase51_authorize_128x128_seed42_longer_run`, with `authorized_next_phase = phase52_controlled_128x128_seed42_longer_run_baseline`, `no_training_in_phase51 = true`, `level4_plus_route_supported = true`, `level5_supported = false`, `no_swe_pinn = true`, and `no_uncontrolled_expansion = true`. Phase 52 should use a recommended 40-epoch cap and compare directly against the Phase 47 `128 x 128` `seed42` 10e baseline. The current Phase 10 boundary-band setting remains `boundary_band_pixels = 1` and `boundary_weight = 2.0`.
...
- make Phase 52 the only authorized next training phase: controlled `128 x 128`, `seed42`, recommended 40-epoch cap, direct comparison with the Phase 47 10e baseline, and no model/loss/config architecture changes
- keep `seed123` / `seed202`, seed replication, `256 x 256`, tile, multiscale, full `500 x 500`, sweeps, and new loss redesign deferred
- follow the compact Phase 45-52 route: Phases 45-50 complete their indexing, feasibility, baseline, diagnostic, warning, and synthesis work; Phase 51 reviewed expansion decision complete; Phase 52 controlled longer-run baseline is the sole authorized next training phase
