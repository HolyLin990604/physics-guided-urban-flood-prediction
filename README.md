...
flowchart TB
    ...
    H["Current Status<br/>Phase 40 next-constraint decision complete<br/>proxy-loss redesign paused<br/>next: no-training SWE data readiness audit<br/>Level 5 SWE/PINN not supported"]:::current
...
flowchart TD
    ...
    C["Strong Physics Boundary<br/>Level 4+ proxy diagnostics supported<br/>Level 5 SWE/PINN not supported<br/>Phases 26 + 30-40"]:::boundary
    ...
    E["Current Decision<br/>pause_loss_redesign_move_to_swe_data_readiness<br/>training_authorized = false<br/>Phase 40 complete"]:::current
    F["Next Research Direction<br/>phase41_swe_data_readiness_audit<br/>no-training data readiness audit"]:::current
...
The next justified follow-up is a no-training SWE data readiness audit, not training, seed expansion, a sweep, further proxy-loss redesign, or post-hoc rescue of the rejected pilot. Phase 38 completed the single authorized `seed42` training run, test evaluation, and guardrail evaluation, with `final_decision = seed42_pilot_rejected`; Phase 39 then completed diagnostic-only trade-off analysis with `phase39_decision = tradeoff_diagnosis_completed_with_missing_optional_inputs`; and Phase 40 selected `pause_loss_redesign_move_to_swe_data_readiness` with `training_authorized = false` and `next_recommended_phase = phase41_swe_data_readiness_audit`. The current Phase 10 boundary-band setting remains `boundary_band_pixels = 1` and `boundary_weight = 2.0`, with Phase 25 target-wet recall consistency as a strong three-seed positive candidate, Phase 26 conservation-proxy diagnostics, Phase 30 synthesis, Phase 31 input recovery readiness, Phase 32 design guardrails, Phase 33 readiness review, Phase 34 threshold formalization, Phase 35 pilot planning, Phase 36 code/smoke-test implementation, Phase 37 authorization review, Phase 38 rejection, Phase 39 diagnosis, and Phase 40 next-constraint decision as the current Level 4+ strong-physics feasibility boundary. Phase 27 remains a mixed seed42 pilot whose primary volume-response objective was not confirmed, Phase 28 remains the diagnostic explanation for why Phase 27 should not be directly expanded, Phase 29 remains a mixed tolerance-band seed42 pilot whose trade-off is not acceptable for confirmation, Phase 39 shows that the `manhole_nonzero_false_dry_guardrail` target/proxy behavior is insufficient under broader guardrails, and Phase 40 pauses proxy-loss redesign after repeated trade-off evidence from Phase 27, Phase 29, and Phase 38.
...
- use a no-training SWE data readiness audit as the next phase
