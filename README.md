...
flowchart TB
    ...
    H["Current Status<br/>Phase 43 full dataset inspection complete<br/>Level 4+ proxy diagnostics supported<br/>metadata recovery / hydrodynamic export needed<br/>Level 5 SWE/PINN not supported"]:::current
...
flowchart TD
    ...
    C["Strong Physics Boundary<br/>Level 4+ proxy diagnostics supported<br/>Level 5 SWE/PINN not supported<br/>Phases 26 + 30-43"]:::boundary
    ...
    E["Current Decision<br/>full_dataset_readiness_uncertain_needs_metadata<br/>training_authorized = false<br/>Phase 43 complete"]:::current
    F["Next Research Direction<br/>metadata recovery / author clarification<br/>ICM / MIKE+ / equivalent export<br/>no training or SWE implementation"]:::current
...
The next justified follow-up is metadata recovery, documentation search, author-side clarification, or hydrodynamic field request/export from InfoWorks ICM, MIKE+, or an equivalent source, not training, seed expansion, a sweep, further proxy-loss redesign, SWE loss implementation, SWE residual implementation, or post-hoc rescue of the rejected pilot. Phase 38 completed the single authorized `seed42` training run, test evaluation, and guardrail evaluation, with `final_decision = seed42_pilot_rejected`; Phase 39 then completed diagnostic-only trade-off analysis with `phase39_decision = tradeoff_diagnosis_completed_with_missing_optional_inputs`; Phase 40 selected `pause_loss_redesign_move_to_swe_data_readiness`; Phase 41 completed the SWE data readiness audit with `readiness_decision = readiness_uncertain_requires_external_data_export`, `level5_supported = false`, `external_hydrodynamic_model_export_needed = true`, and `level4_proxy_supported = true`; Phase 42 completed the hydrodynamic export requirement specification with `selected_decision = export_contract_ready_for_dataset_inspection` and `training_authorized = false`; and Phase 43 inspected UrbanFlood24 full with `selected_decision = full_dataset_readiness_uncertain_needs_metadata`, `level5_supported = false`, `level4_plus_supported = true`, and `training_authorized = false`. The current Phase 10 boundary-band setting remains `boundary_band_pixels = 1` and `boundary_weight = 2.0`, with Phase 25 target-wet recall consistency as a strong three-seed positive candidate, Phase 26 conservation-proxy diagnostics, Phase 30 synthesis, Phase 31 input recovery readiness, Phase 32 design guardrails, Phase 33 readiness review, Phase 34 threshold formalization, Phase 35 pilot planning, Phase 36 code/smoke-test implementation, Phase 37 authorization review, Phase 38 rejection, Phase 39 diagnosis, Phase 40 next-constraint decision, Phase 41 SWE data readiness audit, Phase 42 export data contract, and Phase 43 full dataset inspection as the current Level 4+ strong-physics feasibility boundary. Phase 27 remains a mixed seed42 pilot whose primary volume-response objective was not confirmed, Phase 28 remains the diagnostic explanation for why Phase 27 should not be directly expanded, Phase 29 remains a mixed tolerance-band seed42 pilot whose trade-off is not acceptable for confirmation, Phase 39 shows that the `manhole_nonzero_false_dry_guardrail` target/proxy behavior is insufficient under broader guardrails, Phase 40 pauses proxy-loss redesign, Phase 41 shows that Level 5 SWE/PINN residual constraints are not currently supported, Phase 42 defines requirements without implementing SWE/PINN or claiming Level 5 support, and Phase 43 shows UrbanFlood24 full supports higher-resolution Level 4+ proxy diagnostics but not direct Level 5 SWE/PINN readiness.
...
- prioritize metadata recovery, documentation search, author-side clarification, and/or request/export of fields from InfoWorks ICM, MIKE+, or an equivalent hydrodynamic source next
